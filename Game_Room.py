...
@bot.slash_command(name="отчёт", description="Сообщить о нарушении на сервере")
async def отчёт(ctx: disnake.ApplicationCommandInteraction, пользователь: disnake.Member, причина: str, доказательство: str = None):
    """Отправляет интеграцию в фиксированный канал жалоб"""
    # Получаем фиксированный канал жалоб (замените ID на ваш канал)
    канал = bot.get_channel(1175480769364373725)

    # Предопределенный цвет интеграции
    цвет = "#7d002c"  # Ваш предпочтительный цвет

    # Получаем информацию о пользователе, подавшем жалобу (ctx.author)
    пользователь_заявник = ctx.author

    # Создаем сообщение с отчетом, вставляя картинку перед текстом
    отчет_msg = f"**{('### НАРУШЕНИЯ НА СЕРВЕРЕ').upper()}** На пользователя {пользователь.mention} была подана жалоба от {пользователь_заявник.mention}\n\n**Причина нарушения:**```{причина}```"

    # Добавляем доказательство, если оно предоставлено
    if доказательство:
        отчет_msg += f"\n**Доказательство нарушения:**```{доказательство}```"

    # Создаем Embed с предопределенным цветом
    embed = disnake.Embed(description=отчет_msg, color=int(цвет[1:], 16))

    # Отправляем сообщение в указанный канал с заданным сообщением и цветом
    if канал:
    ...
    # Отправляем сообщение об успешной отправке жалобы пользователю, отправившему жалобу
    await пользователь_заявник.send("Ваше сообщение о нарушении было успешно отправлено: **Команде Game Room: Игроновинки**")
